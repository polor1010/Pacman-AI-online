...
def get_unique_logname():
    """Generate a unique log filename based on current timestamp"""
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    return f"pacman_{timestamp}.log"

# Generate a unique log filename for this run
current_logfile = get_unique_logname()

logname = os.path.join(os.path.dirname(__file__), current_logfile)
logging.basicConfig(filename=logname,
                    filemode='w',  # Changed from 'a' to 'w' since each run gets a new file
                    format='%(asctime)s,%(msecs)03d %(name)s %(levelname)s %(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S',
                    level=logging.DEBUG)

logging.info("Running Pacman Game")
...
class Game:
    ...
    def run(self):
        ...
        while not self.gameOver:
            ...
            # Use the current log filename instead of hardcoded 'pacman.log'
            fname = current_logfile
            f = open(fname, 'a')
            
            if agentIndex > 0 :
                gPos[agentIndex] = self.state.getGhostPosition(agentIndex)
                log = print_to_string('Ghost',agentIndex,gPos[agentIndex])
                ...
                log = print_to_string(observation)
                f.write(log)
                log = print_to_string('t=',counter+1)
                f.write(log)
                log = print_to_string('PACman',pacmanPosition)
                f.write(log)
                f.close()
                #print(observation)
                #print('t=',counter+1)
                #print('PACman',pacmanPosition)
