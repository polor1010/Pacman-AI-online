...
from game import print_to_string, get_unique_logname, current_logfile
...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    ...
    for i in range(numGames):
        ...
        if record:
            import time
            #import pickle
            fname = ('recorded-game-%d.txt' % (i + 1)) + \
                '-'.join([str(t) for t in time.localtime()[1:6]])
            f = open(fname, 'w')
            #components = {'layout': layout, 'actions': game.moveHistory}
            #print(layout)
            #f.write(str(components))  # Use str() instead of pickle.dump()
            f.close()
    ...
    if (numGames-numTraining) > 0:
        ...
        # Use the current log filename from game.py instead of hardcoded 'pacman.log'
        f = open(current_logfile, 'a')

        log = print_to_string('Average Score:', sum(scores) / float(len(scores)))
        f.write(log)
        log = print_to_string('Scores:       ', ', '.join([str(score) for score in scores]))
        f.write(log)
        log = print_to_string('Win Rate:      %d/%d (%.2f)' %
              (wins.count(True), len(wins), winRate))
        f.write(log)
        log = print_to_string('Record:       ', ', '.join(
